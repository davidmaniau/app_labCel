      ...
      <section class="content">
       ...
       // $from = date('Y-m-d', strtotime('-15 day', strtotime($to)));
        ...
        while($row = $query->fetch_assoc()){
         ...
         $sql2x = "SELECT IfNull(sum(".$tablax.".total),0) as totalr, ".$tablax.".venta_id as empid2, ventas.id as empid, IfNull( (select sum(monto) from ventas_detalle_abonos where ventas_detalle_abonos.venta_detalle_id = ".$row['no_doc']." and ventas_detalle_abonos.tipo_doc_id = 1) ,0)
           as totalAbonos, (IfNull( sum(total),0)) - IfNull( (select sum(monto) from ventas_detalle_abonos where ventas_detalle_abonos.venta_detalle_id = ".$row['no_doc']." and ventas_detalle_abonos.tipo_doc_id = 1) ,0)
           as pendiente FROM ".$tablax."
           INNER JOIN ventas on ventas.no_doc = ventas_detalle.venta_id
           where ".$tablax.".venta_id = ".$row['no_doc']." and ventas.tipo_doc_id = 1 and ".$tablax.".tipo_doc_id = 1 ";
           ...
           <tr>
            ...
            <?php if (number_format((($total_detalles+($total_detalles*0)) - $total_abonos),2) > 0 ) { ?>

              <td>
                <span class="right badge badge-danger">PENDIENTE</span> </td>

              <?php }else{ ?>

               <td>
                <span class="right badge badge-success">CANCELADO</span> </td>
              ...
              <td>
                ...
                <a style="color: white; margin-top: 0px;margin-bottom: 5px;" class="btn btn-success btn-sm "  href="<?php echo '../ventas_detalles_r/01index.php?d='.$doc1;  ?>"> <i class="fa fa-check"></i>  Detalles .. </a>
