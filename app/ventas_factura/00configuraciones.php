...
include '../include/session.php';
  ...
  $titulo_pagina = "Venta Facturas ";
  $titulo_modulo = "Venta Facturas ";
...
$dato6 = "estado_registro";
  ...
  $from = date('Y-m-d');//final del rango sea la fecha actual
        ...
        while($row1 = $query->fetch_assoc()){
         ...
         $sql2x = "SELECT IfNull(sum(".$tablax.".total),0) as totalr, ".$tablax.".venta_id as empid2, ventas.id as empid, IfNull( (select sum(monto) from ventas_detalle_abonos where ventas_detalle_abonos.venta_detalle_id = ".$row1['no_doc']." and ventas_detalle_abonos.tipo_doc_id = 2) ,0)
           as totalAbonos, (IfNull( sum(total),0)) - IfNull( (select sum(monto) from ventas_detalle_abonos where ventas_detalle_abonos.venta_detalle_id = ".$row1['no_doc']." and ventas_detalle_abonos.tipo_doc_id = 2) ,0)
           as pendiente FROM ".$tablax."
           INNER JOIN ventas on ventas.no_doc = ventas_detalle.venta_id
           where ".$tablax.".venta_id = ".$row1['no_doc']." and ventas.tipo_doc_id = 2 and ".$tablax.".tipo_doc_id = 2 ";
